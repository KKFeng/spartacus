  ...
  if (domain->dimension == 3) {
    ...
    if (surf->exist) moveptr = &Update::move<1,1>;
      ...
      if (surf->exist) 
      {
          ...
          moveptr2 = &Update::move2<2, 1>;
      }
      else {
          moveptr = &Update::move<2, 0>;
          moveptr2 = &Update::move2<2, 0>;
      }
  }
...
template < int DIM, int SURF > void Update::move2()
{
    bool hitflag;
    int m, icell, icell_original, nmask, outface, bflag, nflag, pflag, itmp, outface_flag;
    int pflag_original, j, jcell;
    int side, minside, minsurf, nsurf, cflag, isurf, exclude, stuck_iterate;
    int pstart, pstop, entryexit, any_entryexit, reaction;
    surfint* csurfs;
    cellint* neigh;
    ...
    while (1) {
        ...
        for (int i = pstart; i < pstop; i++) {
            ...
            for (j = 0; j != 3; j++)
            {
                dx[j] = (random->uniform() - 0.5) * (cells[icell].hi[j] - cells[icell].lo[j]);
                v[j] = dx[j] / dt;
            }
            //v[1] = -1;
            // apply moveperturb() to PKEEP and PINSERT since are computing xnew
...
/*
			...
			if (cflag)
			{
                ...
                if (outface == INTERIOR) {
                    ...
                    //if (DIM == 3) x[2] = xnew[2];
                    //if (cells[icell].proc != me) particles[i].flag = PDONE;
                    //cout << "outface=" << outface_flag << endl;

                    if (cells[icell].proc != me)
                    {
                        icell = icell_original;
                        if (outface_flag % 2 == 0) outface_flag++;
                        else outface_flag--;
                        jcell= cells[icell].neigh[outface_flag];

                        particles[i].Temp = 2*cinfo[icell].Temp- cinfo[jcell].Temp;
                        for (int j = 0; j < 3; j++)
                        {
                            particles[i].macro_v[j] = 2 * cinfo[icell].v[j] - cinfo[jcell].v[j];
                        }
                        break;
                    }
...
/*                        nboundary_one++;
			...
			for (j = 0; j != 2; j++)
			{
				particles[i].x[j] = x_original[j];
			}
            particles[i].flag = pflag_original;
