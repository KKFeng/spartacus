...
SurfCollidehdCLL::SurfCollidehdCLL(SPARTA *sparta, int narg, char **arg) :
...
SurfCollidehdCLL::~SurfCollidehdCLL()
{
  delete [] tstr;
  delete random;
}
...
void SurfCollidehdCLL::init()
{
  ...
  if (tstr) {
    ...
    if (tvar < 0)
      error->all(FLERR,"Surf_collide hdcll variable name does not exist");
    if (!input->variable->equal_style(tvar))
      error->all(FLERR,"Surf_collide hdcll variable is invalid style");
  }
}
...
Particle::OnePart *SurfCollidehdCLL::
...
void SurfCollidehdCLL::hdcll(Particle::OnePart *p, double *norm)
{
